...
const FeedbackList = () =>  {
    ...
    useEffect(() => {
        getFeedback();
    }, []); //added an empty *dependency array here so getFeedback will only run on the first load, not everytime the function runs. (not sure why that means that though...)
...
return (
    <div className="feedback-list">
            {fetchedFeedback.map((item) => (
                <FeedbackCard 
                    key={item.id}
            {/*  key={item.id}) helps react track what items changed */}
                    title={item.title}
